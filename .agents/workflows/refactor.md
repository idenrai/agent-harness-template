...
| :--- | :--- | :--- | :--- |
| **구조 리팩토링 (Macro)** | `project-structure-refactoring` | `.agents/skills/project-structure-refactoring/SKILL.md` | 도메인별 폴더 응집(Package-by-Feature), 레이어 경계 관리, Barrels 경계 관리, `git mv` 기반 히스토리 보존, Junk Drawer 제거 |
...
| **타입 & 코딩 표준 (Safety)** | `project-context` | `.agents/rules/project-context.md` | 프로젝트 언어의 타입 안전성 유지, 엄격 모드(`strict`) 준수, 런타임 스키마 검증 |
| **프레임워크 최적화 (Engine)** | `project-context` | `.agents/rules/project-context.md` | 프로젝트 프레임워크 모범 사례 준수, 컴포넌트 렌더링/메모리/네트워크 최적화 |
...
2. ❌ **그린 베이스라인 필수 (No Refactor on Red):**
   - 리팩토링 착수 전 반드시 프로젝트 테스트/빌드 검증을 실행하여 통과(Green) 상태임을 확인합니다.
...
- 작업 전 반드시 `.agents/rules/project-context.md`에 설정된 빠른 검증 명령어를 실행하여 정상 동작 여부를 확인합니다 (예: `npm test`, `pytest`, `go test` 등).
...
- **Macro Level (구조적 문제):**
  ...
  - *Misplaced Module:* 도메인에 맞지 않는 폴더에 모듈/컴포넌트가 방치됨.
  - *Barrels Anti-pattern:* 모든 서브폴더마다 배럴(`index.*`)을 두고 순환 참조를 유발함.
  - *Large Monolithic File:* 단일 파일 안에 수백 줄의 서로 다른 책임이 강결합됨.
- **Micro Level (코드 레벨 문제):**
  - *Bloaters:* 지나치게 긴 함수(Long Method), 많은 인자를 받는 매개변수 목록(Long Parameter List), 거대한 클래스/모듈.
  - *Dispensables:* 사용되지 않는 데드 코드(Dead Code), 무의미한 임시 변수, 코드 기능을 설명하려는 주석.
  - *Change Preventers:* 하나의 변경이 여러 파일로 번지는 산탄총 수술(Shotgun Surgery), 단일 모듈이 여러 이유로 변경되는 분산 변경(Divergent Change).
  - *Conditional Complexity:* 깊게 중첩된 조건문 및 거대한 분기 블록.
  - *Primitive Obsession:* 도메인 개념을 순수 원시값(Primitive)으로만 다루는 행위.
...
- 논리적으로 독립된 단위로 스테이지(Stage)를 나누어 진행합니다:
  - **Stage N:** 파일 이동 (`git mv`) ➡️ import 경로 조정 ➡️ 빠른 검증(Lint/Test) ➡️ Git Commit.
  - **Stage N+1:** 함수 추출 및 로직 단순화 ➡️ 빠른 검증(Lint/Test) ➡️ Git Commit.
- 각 단계가 끝날 때마다 빌드와 테스트가 100% Green이어야 합니다.

### Step 5. 사후 전수 증명 및 롤백 가드 (Verification Proof)
- 변경이 완료되면 전체 빌드 및 통합 테스트 파이프라인을 실행합니다 (예: `npm run build`, `npm test` 등).
...
function getStatus(item: Item | null, isActive: boolean) {
  if (item) {
    if (isActive) {
...
function getStatus(item: Item | null, isActive: boolean): ItemStatus {
  if (!item) return "not-found"
  if (!isActive) return "inactive"
  if (item.entries.length === 0) return "empty"
  return "ready"
}
```
...
function processPayload(rawContent: string) {
  // 1. JSON 파싱 및 기본 검사
...
function parseRawPayload(rawContent: string): unknown {
  const json = JSON.parse(rawContent)
  if (!json || typeof json !== "object") throw new Error("Invalid format")
  return json
}

function validatePayloadItemCount(items: unknown): asserts items is unknown[] {
...
- [ ] **5. Flat Conditionals:** 중첩된 조건문이 제거되고 Guard Clause나 매핑 테이블로 평탄화되었는가?
- [ ] **6. History Preservation:** 파일 이동 시 `git mv`를 사용하여 커밋 히스토리가 보존되었는가?
- [ ] **7. Non-Breaking Public API:** 외부에서 참조하는 공개 인터페이스가 파괴되지 않았는가?
